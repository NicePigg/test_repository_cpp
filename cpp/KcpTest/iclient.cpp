...
void loop(kcpObj *send) {
    ...
    int i = 0;
    while(1) {
        //ikcp_update包含了ikcp_flush，ikcp_flush将发送队列中的数据通过下层协议UDP进行发送
        ikcp_update(send->pkcp,iclock());//不是调用一次两次就起作用，要loop调用
        isleep(100);
        ret = ikcp_send(send->pkcp, (char *) &buf, sizeof(buf));
        if(ret < 0) {
            printf("send %d failed, ret:%d, size:%ld\n", i, ret, sizeof(buf));
            return;
        }
        ...
        ret = ikcp_recv(send->pkcp, (char *)&recvbuf, sizeof(recvbuf));
